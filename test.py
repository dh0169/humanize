...
def test_handle_join(app, websockets_client, api_test_client):
    try:
        ...
        response = api_test_client.get('/api/lobby/sessions')
        print("\Getting available sessions...\n")
        print(response.json)
        ...
        assert received[0]['args']['message'] == 'User is not registered'
        ...
        received = websockets_client_after_registering_user.get_received('/chat')[1]['args']
        print(received)
        assert received['message'] == 'user 1/2 joined!'
